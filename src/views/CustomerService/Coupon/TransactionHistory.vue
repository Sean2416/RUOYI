      ...
      <el-form status-icon class="queryBlock" size="mini" :model="query" :rules="rules" ref="query">
        <el-form-item label="身分證號碼" prop="identity">
          <el-input placeholder="请输入内容" v-model="query.identity" class="input-with-select" clearable>
            <el-button slot="append" icon="el-icon-search" @click="getTransInfo"></el-button>
          </el-input>
        </el-form-item>
        ...
        <el-form-item label="自訂區間" >
          <el-date-picker v-model="query.date" value-format="yyyy/MM/dd" type="daterange" range-separator="至" start-placeholder="開始日期"  :disabled="query.interval !== 0 "
